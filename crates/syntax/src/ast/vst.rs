...
pub use super::{HasAttrs, generated};
...
impl TryFrom<generated::nodes::AssertForallExpr> for AssertForallExpr {
    ...
    fn try_from(item: generated::nodes::AssertForallExpr) -> Result<Self, Self::Error> {
        // `ClosureExpr` is also an `Expr`, so the generated singular `expr()` accessor
        // returns the quantified closure rather than the expression after `implies`.
        let conclusion = item.implies_token().and_then(|implies| {
            item.syntax()
                .children()
                .filter_map(generated::nodes::Expr::cast)
                .find(|expr| expr.syntax().text_range().start() >= implies.text_range().end())
        });
        Ok(Self {
            attrs: item
                .attrs()
