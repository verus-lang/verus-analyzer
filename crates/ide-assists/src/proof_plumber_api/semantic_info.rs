...
use hir::Semantics;
use syntax::ast::{self, HasModuleItem, vst};

impl<'a, 'db> AssistContext<'a, 'db> {
    /// From an VST Expr, get the definition VST Adt of that type
    pub(crate) fn type_of_expr_adt(&self, expr: &vst::Expr) -> Option<vst::Adt> {
    ...
    pub(crate) fn type_of_expr_enum(&self, expr: &vst::Expr) -> Option<vst::Enum> {
    ...
    pub(crate) fn type_of_pat_enum(&self, pat: &vst::Pat) -> Option<vst::Enum> {
        let sema: &Semantics<'_, ide_db::RootDatabase> = &self.sema;
        let hir_ty: Vec<hir::Type<'_>> =
    ...
    pub(crate) fn type_of_expr_struct(&self, expr: &vst::Expr) -> Option<vst::Struct> {
        let typename = self.type_of_expr_adt(expr)?;
    ...
    pub(crate) fn resolve_type_enum(&self, ty: &vst::Type) -> Option<vst::Enum> {
        let sema: &Semantics<'_, ide_db::RootDatabase> = &self.sema;
        let hir_ty: Vec<hir::Type<'_>> =
    ...
    pub(crate) fn name_ref_from_call_expr(&self, call: &vst::CallExpr) -> Option<vst::NameRef> {
    ...
    pub(crate) fn vst_find_fn(&self, call: &vst::CallExpr) -> Option<vst::Fn> {
        for item in self.source_file().items() {
            let Ok(v_item): Result<ast::generated::vst_nodes::Item, _> = item.try_into() else {
                continue;
            };
            match v_item {
