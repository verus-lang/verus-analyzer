...
pub(crate) fn intro_failing_ensures(acc: &mut Assists, ctx: &AssistContext<'_, '_>) -> Option<()> {
    ...
    acc.add(
        AssistId("intro_failing_ensures", AssistKind::RefactorRewrite, None),
...
pub(crate) fn vst_rewriter_intro_failing_ensures(
    ...
    let post_fails = filter_post_failures(&ctx.verus_errors_inside_fn(&this_fn)?);
