...
use syntax::{
    T,
    ast::{self, AstNode, CmpOp, Ordering, vst::*},
};
...
/// assert forall ||  A == B ==> Q
/// assert forall ||  A <= B ==> Q
pub(crate) fn split_smaller_or_equal_to(
    acc: &mut Assists,
    ...
    let v_conclusion = *assert.expr.clone()?;
    ...
    acc.add(
        AssistId("split_smaller_or_equal_to", AssistKind::RefactorRewrite, None),
