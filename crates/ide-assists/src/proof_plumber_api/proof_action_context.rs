...
impl<'a, 'db> AssistContext<'a, 'db> {
    ...
    /// Get precondition failures that were generated by calling this function
    pub(crate) fn pre_failures_by_calling_this_fn(
        &self,
        func: &vst::Fn,
    ) -> Option<Vec<PreFailure>> {
        let surrounding_fn: &ast::Fn = func.cst.as_ref()?;
        let surrounding_range: syntax::TextRange = surrounding_fn.syntax().text_range();
    ...
    pub(crate) fn post_failures(&self) -> Vec<PostFailure> {
        filter_post_failures(self.verus_errors())
    }
    ...
    pub(crate) fn replace_statement<ST0, ST1>(
        &self,
        func: &vst::Fn,
        old: ST0,
        new: ST1,
