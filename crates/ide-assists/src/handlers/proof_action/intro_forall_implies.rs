...
use syntax::{
    T,
    ast::{self, AstNode, LogicOp, vst::*},
};

/// Change `assert(forall || P ==> Q)` into
/// `assert forall || P implies Q`
pub(crate) fn intro_forall_implies(acc: &mut Assists, ctx: &AssistContext<'_, '_>) -> Option<()> {
    ...
    acc.add(
        AssistId("intro_forall_implies", AssistKind::RefactorRewrite, None),
