...
pub fn filter_pre_failuires(verus_errors: impl AsRef<[VerusError]>) -> Vec<PreFailure> {
    let mut pre_errs = vec![];
    for verr in verus_errors.as_ref() {
...
pub fn filter_post_failures(verus_errors: impl AsRef<[VerusError]>) -> Vec<PostFailure> {
