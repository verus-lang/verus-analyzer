...
export class Ctx implements RustAnalyzerExtensionApi {
    ...
    private async getOrCreateClient() {
        ...
        if (!this.traceOutputChannel) {
            this.traceOutputChannel = new LazyOutputChannel("verus-analyzer LSP Trace");
...
/**
 * Pattern to detect MSRV warning messages from the verus-analyzer server.
 */
const MSRV_WARNING_PATTERN = /using an outdated toolchain version.*verus-analyzer only supports/is;
...
export async function handleMsrvWarning(message: string): Promise<boolean> {
    ...
    const result = await vscode.window.showWarningMessage(
        "Your workspace uses a rust-toolchain file with a toolchain too old for the extension shipped verus-analyzer to work properly. " +
            "Consider adding the verus-analyzer component to the toolchain file to use a compatible verus-analyzer version. " +
            "Add the following to your rust-toolchain file's `[toolchain]` section:\n" +
            'components = ["verus-analyzer"]',
        { modal: true },
        openFile,
    );
