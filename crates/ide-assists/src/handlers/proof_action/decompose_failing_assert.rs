//! Decomposes a failing Verus assertion into smaller proof obligations.
...
pub(crate) fn localize_error(acc: &mut Assists, ctx: &AssistContext<'_, '_>) -> Option<()> {
    ...
    acc.add(
        AssistId("localize_error", AssistKind::RefactorRewrite, None),
...
fn split_expr(ctx: &AssistContext<'_, '_>, exp: &Expr) -> Option<Vec<Expr>> {
    match exp {
        Expr::BinExpr(be) => {
            match be.op {
