...
use super::ast_src::{AstSrc, Field, KindsSrc};
use itertools::Itertools;
use quote::{format_ident, quote};

use crate::codegen;

fn field_name(field: &Field) -> proc_macro2::Ident {
    format_ident!("{}", field.method_name())
}

fn accessor_name(field: &Field) -> proc_macro2::Ident {
    let name = field.method_name();
    format_ident!("{}", if name == "op" { "op_token" } else { &name })
}
...
pub(crate) fn generate_vst(_kinds: KindsSrc, grammar: &AstSrc) -> String {
    ...
    let from_node_to_vnode_struct: Vec<_> = grammar
        ...
        .map(|node| {
            ...
            let fields = node.fields.iter().map(|field| {
                let name = field_name(field);
                let accessor = accessor_name(field);
                let ty = field.ty();

                if field.is_many() {
                    quote! {
                        #name : item.#accessor().into_iter().map(#ty::try_from).collect::<Result<Vec<#ty>, String>>()?,
                    }
                } else if field.token_kind().is_some() {
                    // hacky for now
                    // maybe special-case identifier to "#name : Option<String>"
                    ...
                    if field.stores_token_text() {
                        // #name : Option<String>,
                        quote! {
                            #name : item.#accessor().map(|it| it.text().to_string()),
                        }
                    } else {
                        // #name : bool,
                        quote! {
                            #name : item.#accessor().is_some(),
                        }
                    }
                } else {
                    if field.is_one() {
                        // pub #name : Box<#ty>,
                        quote! {
                            #name: Box::new(item.#accessor().ok_or(format!("{}", stringify!(#name))).map(|it| #ty::try_from(it))??),
                        }
                    } else {
                        // pub #name : Option<Box<#ty>>,
                        quote! {
                            #name: match item.#accessor() {
    ...
    codegen::add_preamble(crate::flags::CodegenType::Grammar, codegen::reformat(ast.to_string()))
}
